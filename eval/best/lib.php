...
class workshep_best_evaluation extends workshep_evaluation {
     ...
     * Constructor
     *
     * @param workshep $workshep The workshep api instance
     * @return void
     */
    public function __construct(workshep $workshep) {
        global $DB;
        $this->workshep = $workshep;
        $this->settings = $DB->get_record('workshepeval_best_settings', array('workshepid' => $this->workshep->id));
    }

    /**
     * Calculates the grades for assessment and updates 'gradinggrade' fields in 'workshep_assessments' table
     *
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        if (empty($this->settings)) {
            ...
            $DB->insert_record('workshepeval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('workshepeval_best_settings', 'comparison', $settings->comparison,
    ...
    public static function delete_instance($workshepid) {
        ...
        $DB->delete_records('workshepeval_best_settings', array('workshepid' => $workshepid));
    }
    ...
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings) {
        ...
        if (is_null($average)) {
            foreach ($assessments as $asid => $assessment) {
                if (!is_null($assessment->gradinggrade)) {
                    $DB->set_field('workshep_assessments', 'gradinggrade', null, array('id' => $asid));
                }
            }
            return;
        }
        ...
        foreach ($grades as $assessmentid => $grade) {
            if (grade_floats_different($grade, $assessments[$assessmentid]->gradinggrade)) {
                ...
                $DB->update_record('workshep_assessments', $record, true);  // bulk operations expected
            }
        }
 ...
 */
class workshep_best_evaluation_settings_form extends workshep_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $mform->addElement('select', 'comparison', get_string('comparison', 'workshepeval_best'), $options);
