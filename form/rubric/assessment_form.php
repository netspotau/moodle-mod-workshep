 ...
 */
abstract class workshep_rubric_assessment_form extends workshep_assessment_form {
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            if (empty($data['chosenlevelid__idx_'.$i])) {
                $errors['chosenlevelid__idx_'.$i] = get_string('mustchooseone', 'workshepform_rubric'); // used in grid
                $errors['levelgrp__idx_'.$i] = get_string('mustchooseone', 'workshepform_rubric');      // used in list
            }
        }
        return $errors;
    }
 ...
 */
class workshep_rubric_list_assessment_form extends workshep_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'workshepform_rubric', $i+1);
 ...
 */
class workshep_rubric_grid_assessment_form extends workshep_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $numofcolumns = array_reduce($levelcounts, 'workshep::lcm', 1);

        $mform->addElement('header', 'rubric-grid-wrapper', get_string('layoutgrid', 'workshepform_rubric'));

        $mform->addElement('html', '<table class="rubric-grid">' . "\n");
        $mform->addElement('html', '<th class="header">' . get_string('criteria', 'workshepform_rubric') . '</th>');
        $mform->addElement('html', '<th class="header" colspan="'.$numofcolumns.'">'.get_string('levels', 'workshepform_rubric').'</th>');
