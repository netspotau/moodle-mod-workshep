 ...
 * @uses moodleform
 */
class workshep_edit_rubric_strategy_form extends workshep_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'workshepform_rubric', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'workshepform_rubric'), '', $descriptionopts);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $mform->addGroup($levelgrp, 'level__idx_'.$i.'__idy_'.$j, get_string('levelgroup', 'workshepform_rubric'), array(' '), false);
        ...
        $mform->addElement('submit', 'adddims', get_string('addmoredimensions', 'workshepform_rubric',
                workshep_rubric_strategy::ADDDIMS));
        $mform->closeHeaderBefore('adddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'workshepform_rubric'));
        $layoutgrp = array();
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutlist', 'workshepform_rubric'), 'list');
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutgrid', 'workshepform_rubric'), 'grid');
        $mform->addGroup($layoutgrp, 'layoutgrp', get_string('layout', 'workshepform_rubric'), array('<br />'), false);
