...
$dimensions = $workshep->grading_strategy_instance()->get_dimensions_info();
...
$examples = $workshep->get_examples_for_manager();

// $assessments[reviewerid][submissionid]
$assessments = array();
$assessments_rs = $workshep->grading_strategy_instance()->get_assessments_recordset(null, true);
...
if($teammode) {
    ...
    $submissions = $workshep->get_submissions();
}
...
if (count($examples)) {
    ...
    $exampletotals = $DB->get_records_select("workshep_assessments", "submissionid $select", $params);
...
$feedbackset = $DB->get_records_select("workshep_assessments", "submissionid $select", $params);
...
foreach($h as $i) {
    $headers[$i] = get_string($i, 'workshep');
}
...
$csv->add_data(array( get_string('overallmarks', 'workshep') ));
...
foreach($examples as $ex) {
    $row = array("markername" => get_string('referencemarker','workshep'));
...
if (!$teammode) {
    $headers["submitteridnumber"] = get_string("submitteridnumber", 'workshep');
}
...
foreach($dimensions as $dim) {
    ...
    if (! empty($needs_comments[$dim->id])) {
        $headers["comment$dim->id"] = get_string('comments', 'workshep');
    }
}

if ($needs_feedback == true) {
    $headers["feedback"] = get_string('feedback', 'workshep');
}
$headers["overallmark"] = get_string('overallmark', 'workshep') . ' / ' . $maximumscore;
$headers["scaledmark"] = get_string('scaledmark', 'workshep') . ' / 100';

if ($teammode) {
    $headers["gradinggrade"] = get_string("gradinggrade", "workshep");
}

$csv->add_data(array( get_string('individualmarks', 'workshep') ));
