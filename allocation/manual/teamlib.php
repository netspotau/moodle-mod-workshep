...
class workshep_teammode_manual_allocator extends workshep_manual_allocator {
    ...
    public function ui() {
        ...
        // fetch the list of ids of all workshep participants - this may get really long so fetch just id
        ...
        if($this->workshep->cm->groupingid) {
            $groupinggroups = groups_get_all_groups($this->workshep->cm->course, 0, $this->workshep->cm->groupingid, 'g.id');
...
WHERE g.courseid = {$this->workshep->cm->course} AND m.userid $insql $groupingsql
 ...
 */
class workshepallocation_teammode_manual_allocations implements renderable {
    public $allocations;
    public $gradeitems;
    public $userinfo;
