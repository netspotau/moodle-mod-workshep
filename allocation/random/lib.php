 ...
 * Allocates the submissions randomly
 */
class workshep_random_allocator implements workshep_allocator {
     ...
     *
     * @param workshep_random_allocator_setting $setting
     * @param workshep_allocation_result allocation result logger
     */
    public function execute(workshep_random_allocator_setting $settings, workshep_allocation_result $result) {
        ...
        $reviewers      = $this->workshep->get_grouped($reviewers);
        $assessments    = $this->workshep->get_all_assessments();
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'workshepallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'workshepallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'workshepallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->workshep->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'workshepallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'workshepallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    ...
                    $result->log(get_string('assessmentdeleteddetail', 'workshepallocation_random', $a), 'info', 1);
                }
            }
            $this->workshep->delete_assessment($delassessments);
        }
    ...
    public function ui() {
        ...
        if ($m == self::MSG_SUCCESS) {
            $message->set_text(get_string('randomallocationdone', 'workshepallocation_random'));
            $message->set_type(workshep_message::TYPE_OK);
        }
        ...
        if (VISIBLEGROUPS == $gmode or SEPARATEGROUPS == $gmode) {
            $nogroupusers = $this->workshep->get_ungrouped_users(); //this function exists for a reason
            if (!empty($nogroupusers)) {
                ...
                $a = implode(', ', $list);
                $out .= $output->box(get_string('nogroupusers', 'workshepallocation_random', $a), 'generalbox warning nogroupusers');
            }
        }

        // TODO $out .= $output->heading(get_string('stats', 'workshepallocation_random'));
     ...
     *
     * @param array $authors grouped of {@see workshep::get_potential_authors()}
     * @param array $reviewers grouped by {@see workshep::get_potential_reviewers()}
     * @param array $assessments as returned by {@see workshep::get_all_assessments()}
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        foreach ($newallocations as $newallocation) {
            ...
            $status = $this->workshep->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (workshep::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'workshep');
                }
     ...
     * @param workshep_allocation_result $result allocation result logger
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (workshep_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            // circles are authors, squares are reviewers
            $result->log(get_string('resultnumperauthor', 'workshepallocation_random', $numofreviews), 'info');
            $allcircles = $authors;
            $allsquares = $reviewers;
            // get current workload
            list($circlelinks, $squarelinks) = $this->convert_assessments_to_links($assessments);
        } elseif (workshep_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            $result->log(get_string('resultnumperreviewer', 'workshepallocation_random', $numofreviews), 'info');
            $allcircles = $reviewers;
            $allsquares = $authors;
            // get current workload
            list($squarelinks, $circlelinks) = $this->convert_assessments_to_links($assessments);
        } else {
            throw new moodle_exception('unknownusertypepassed', 'workshep');
        }
                    ...
                    while ($keeptrying && (count($circlelinks[$circleid]) < $requiredreviews)) {
                        ...
                        if (NOGROUPS == $gmode) {
                            if (in_array(0, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeers', 'workshepallocation_random'), 'error', 1);
                                break;
                            }
                            $targetgroup = 0;
                            ...
                            if (in_array($circlegroupid, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeersingroup', 'workshepallocation_random'), 'error', 1);
                                break;
                            }
        ...
        if (workshep_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            foreach ($circlelinks as $circleid => $squares) {
                foreach ($squares as $squareid) {
     ...
     *
     * @param array $assessments array of assessments as returned by {@link workshep::get_all_assessments()}
 ...
 */
class workshep_random_allocator_setting {
     ...
     * @return workshep_random_allocator_setting
     */
